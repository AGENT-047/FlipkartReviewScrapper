...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            new_link_box = prod_html.findAll("div", {"class": "col JOpGWq"})
            new_box = new_link_box[0].findAll('a',href=True)[-1]
            new_link = "https://www.flipkart.com" + new_box.get('href')
            new_res = requests.get(new_link)
            new_html= bs(new_res.text, "html.parser")

            # parsing the product page as HTML
            commentboxes = new_html.find_all('div', {'class': "_1AtVbE col-12-12"})
            commentboxes = commentboxes[4:]# finding the HTML section containing the customer comments
